...
import { BsTelephone, BsCameraVideo, BsX, BsImage } from "react-icons/bs";
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 h-[70vh] rounded-t-3xl bg-[var(--fb-card)] shadow-[var(--fb-shadow)] md:hidden">
      <div className="flex items-center justify-between border-b border-[var(--fb-divider)] px-4 py-3">
        <div className="flex items-center gap-3">
          <div className="relative h-10 w-10 overflow-hidden rounded-full bg-slate-500">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--fb-muted)]">
      ...
      <div
        ref={messageListRef}
        className="flex h-[calc(70vh-146px)] flex-col gap-3 overflow-y-auto px-4 py-4"
      >
        {isLoading ? (
          <div className="space-y-2">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={`chat-drawer-loading-${index}`}
