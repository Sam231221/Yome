...
export const ProfileDropDown = ({
  ...
  const ProfileHeadRef = useRef<HTMLDivElement | null>(null);
  const ProfileBodyRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    let handler = (e) => {
      if (
        ProfileHeadRef.current &&
        ProfileBodyRef.current &&
        !ProfileHeadRef.current.contains(e.target as Node) &&
        !ProfileBodyRef.current.contains(e.target as Node)
      ) {
        setOpen(false);
      }
    };
